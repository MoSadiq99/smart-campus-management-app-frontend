...
export class ResourceService {
  ...
  createResource(resource: ResourceCreateDto): Observable<ResourceDto> {
    return this.http
      .post<ResourceDto>(this.apiUrl, resource, {
  ...
  getResourceById(id: number): Observable<ResourceDto> {
    return this.http.get<ResourceDto>(`${this.apiUrl}/${id}`).pipe(catchError(this.handleError));
  }

  // Get all resources
  getAllResources(): Observable<ResourceDto[]> {
    return this.http.get<ResourceDto[]>(this.apiUrl).pipe(catchError(this.handleError));
  }

  // Update a resource
  updateResource(id: number, resource: ResourceCreateDto): Observable<ResourceDto> {
    return this.http.put<ResourceDto>(`${this.apiUrl}/${id}`, resource).pipe(catchError(this.handleError));
  }
